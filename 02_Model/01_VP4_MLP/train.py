...
def train_for_target(target):
    ...
    #             if fname.startswith("checkpoint") and os.path.isdir(p):
    #                 shutil.rmtree(p)

    # logger.info(f"[{target}] Best Fold F1: {best_f1}")

    # ========== 测试集评估（新增 MCC、ROC-AUC、PR-AUC） ==========
        ...
        if len(existing_labels) >= 2:
            ...
            try:
                roc_auc_macro = roc_auc_score(
                    y_true_bin, y_score_sub, average="macro", multi_class="ovr"
                )
            ...
            try:
                # PR-AUC 使用 macro 平均的 AP（Average Precision）
                pr_auc_macro = average_precision_score(
                    y_true_bin, y_score_sub, average="macro"
                )
            except Exception as e:
                pr_auc_macro = np.nan
    ...
    logger.info(
        ...
        f"F1 Mean: {np.nanmean(df_metrics['macro_f1']):.4f}, "
        f"MCC Mean: {np.nanmean(df_metrics['mcc']):.4f}, "
        f"ROC-AUC Mean: {np.nanmean(df_metrics['roc_auc_macro']):.4f}, "
        f"PR-AUC Mean: {np.nanmean(df_metrics['pr_auc_macro']):.4f}"
    )

...
DISPLAY_NAMES = {
    "02_VP7_MLP": "VP7-MLP",
    "02_VP7_SE": "VP7-SE",
    "02_VP7_SE_Maxmean": "VP7-SE (MaxMean)",
    "02_VP7_SE_Maxmean_Pretraining": "VP7-SE (MaxMean, Pretrain)",
}
...
COLORS = ["#84C3B7", "#7DA6C6", "#E68B81","#EAAA60"]

OUT_PNG = "vp7_fold_metrics_bar_scatter_error.png"
OUT_PDF = "vp7_fold_metrics_bar_scatter_error.pdf"

# ================== 读取函数 ==================
def find_csv(root: str) -> str | None:
    for sub in CANDIDATE_DIRS:
        p = Path(root) / sub / "VP7" / CSV_NAME
        if p.is_file():
            return str(p)
    return None
...
for i, root in enumerate(present_roots):
    ...
    for j, metric in enumerate(METRICS):
        ...
        plt.scatter(xs, vals, s=24, facecolors="white", edgecolors=color, linewidths=1.0, zorder=3)

    # === 只给 VP7-SE (MaxMean, Pretrain) 标注数值 ===
    if root == "02_VP7_SE_Maxmean_Pretraining":
        for bar in bars:
...
plt.ylim(0.8, 1.02)
plt.title("VP7 Models(5-Fold)", fontsize=14)
plt.legend(frameon=False, fontsize=12, ncol=min(2, n_methods))

...
def train_for_target(target):
    ...
    logger.info(f"========== Start Training for {target} (ALL data, no folds) ==========")
    ...
    # === 保存“最后一轮”的模型权重 ===
    final_model_dir = os.path.join(output_dir, config["best_model_path"])
    os.makedirs(final_model_dir, exist_ok=True)
    torch.save(trainer.model.state_dict(), os.path.join(final_model_dir, "pytorch_model.bin"))
    logger.info(f"✅ 最后一轮模型权重已保存到 {final_model_dir}")

    # ========== 测试集评估 ==========
    test_ds = HostDataset(test_path, tokenizer, max_len, label2idx)
    logger.info(f"\n[🔍 Test on {target}] Evaluate final model on test.csv")
    ...
    if len(existing_labels) >= 2:
        y_true_bin = label_binarize(labels, classes=existing_labels)
        y_score_sub = probs[:, existing_labels]
        try:
...
if __name__ == "__main__":
    targets = ["VP7"]  # 需要时可改为 ["VP4"] 或 ["VP4","VP7"]
